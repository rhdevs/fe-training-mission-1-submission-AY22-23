...
let students: StdRecord[] = [bob, steve, mary] // assign an appropriate type to `students`

// convert to anonymous function
// use string literals
// add appropriate type declarations to function args
const printNameAndYear = (student: StdRecord) => {
  let text = `${student.name} is year ${student.year}` 
  console.log(text)
}
const introduceSelf = (student: StdRecord) => {
  let text =
...
newNumbers = numbers.map((index, x) => index + x)
console.log('newNumbers:', newNumbers)
